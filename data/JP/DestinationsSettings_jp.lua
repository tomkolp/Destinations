...
do
  ...
  Add("DEST_SETTINGS_ALL_SHOW_ENGLISH",
    "英語名の地点を表示する") -- Should read "Show english POI names" on all non-english clients.
  ...
  Add("DESTCOMMANDdqed",
    "/dqed (Destinations クエストエディタ) : このコマンドはクエストのピンを編集するかどうかをトグルします。チャットに on または off が表示されます。ON にしている時はマップを開いて、ピンをクリックすると完了または未完了を指定できます。編集が完了したらOFFにするためにもう一度このコマンドを実行します!")
